...
function sagepay_ipn() {
	...
	if ( isset($_GET['crypt']) && !empty($_GET['crypt']) && $_GET['sagepay'] == 'ipn') {
		...
		if ( $return['Status'] == 'OK' || $return['Status'] == 'AUTHENTICATED'|| $return['Status'] == 'REGISTERED' ) {
			...
			edd_insert_payment_note( $payment[2], sprintf( __('SagePay Payment Completed and VPSTxId is %s', 'spxml_patsatech'), $return['VPSTxId'] ) );
...
function sagepay_add_settings($gateway_settings) {
	...
	$sagepay_settings  = apply_filters( 'edd_sagepay_settings', $sagepay_settings );

	$gateway_settings['sagepay'] = $sagepay_settings;

	return $gateway_settings;
}
add_filter('edd_settings_gateways', 'sagepay_add_settings');

function edd_register_sagepay_gateway_section( $gateway_sections ) {
	$gateway_sections['sagepay'] = __( 'SagePay Form', 'sagepay_patsatech' );
	return $gateway_sections;
}
add_filter( 'edd_settings_sections_gateways', 'edd_register_sagepay_gateway_section' );
...
function encryptAndEncode($strIn) {
	...
	return "@".bin2hex(openssl_encrypt($strIn, 'AES-128-CBC', $edd_options['sagepay_vendor_password'], OPENSSL_RAW_DATA, $edd_options['sagepay_vendor_password']));
}
...
function decodeAndDecrypt($strIn) {
	...
	return openssl_decrypt($strIn, 'AES-128-CBC', $edd_options['sagepay_vendor_password'], OPENSSL_RAW_DATA, $edd_options['sagepay_vendor_password']);
}
